...
#ifndef CXX_STR_REGEX_H
#define CXX_STR_REGEX_H
...
namespace cxx {
    namespace str {
        ...
        // Regexp
        //
        template <class CHART> class regexpT
        {
        public:
            regexpT(const CHART * pattern = 0, int flags = 0);
            regexpT(const CHART * pattern, int length, int flags);
        ...
        // Implementation
        //
        template <class CHART> regexpT <CHART> :: regexpT(const CHART * pattern, int flags)
        {
            Compile(pattern, CBufferRefT<CHART>(pattern).GetSize(), flags);
        }

        template <class CHART> regexpT <CHART> :: regexpT(const CHART * pattern, int length, int flags)
        {
            Compile(pattern, length, flags);
        }

        template <class CHART> inline void regexpT <CHART> :: Compile(const CHART * pattern, int flags)
        {
            Compile(pattern, CBufferRefT<CHART>(pattern).GetSize(), flags);
        }

        template <class CHART> void regexpT <CHART> :: Compile(const CHART * pattern, int length, int flags)
        {
            m_builder.Clear();
            if(pattern != 0) m_builder.Build(CBufferRefT<CHART>(pattern, length), flags);
        }

        template <class CHART> inline MatchResult regexpT <CHART> :: MatchExact(const CHART * tstring, CContext * pContext) const
        {
            return MatchExact(tstring, CBufferRefT<CHART>(tstring).GetSize(), pContext);
        }

        template <class CHART> MatchResult regexpT <CHART> :: MatchExact(const CHART * tstring, int length, CContext * pContext) const
        {
        ...
        template <class CHART> MatchResult regexpT <CHART> :: Match(const CHART * tstring, int start, CContext * pContext) const
        {
            return Match(tstring, CBufferRefT<CHART>(tstring).GetSize(), start, pContext);
        }

        template <class CHART> MatchResult regexpT <CHART> :: Match(const CHART * tstring, int length, int start, CContext * pContext) const
        {
            if(m_builder.m_pTopElx == 0)
                return 0;
        ...
        template <class CHART> MatchResult regexpT <CHART> :: Match(CContext * pContext) const
        {
            if(m_builder.m_pTopElx == 0)
                return 0;
        ...
        template <class CHART> inline CContext * regexpT <CHART> :: PrepareMatch(const CHART * tstring, int start, CContext * pContext) const
        {
            return PrepareMatch(tstring, CBufferRefT<CHART>(tstring).GetSize(), start, pContext);
        }

        template <class CHART> CContext * regexpT <CHART> :: PrepareMatch(const CHART * tstring, int length, int start, CContext * pContext) const
        {
            if(m_builder.m_pTopElx == 0)
                return 0;
        ...
        template <class CHART> inline int regexpT <CHART> :: GetNamedGroupNumber(const CHART * group_name) const
        {
            return m_builder.GetNamedNumber(group_name);
        }

        template <class CHART> CHART * regexpT <CHART> :: Replace(const CHART * tstring, const CHART * replaceto, int start, int ntimes, MatchResult * result, CContext * pContext) const
        {
            int result_length = 0;
        ...
        template <class CHART> CHART * regexpT <CHART> :: Replace(const CHART * tstring, int string_length, const CHART * replaceto, int to_length, int & result_length, int start, int ntimes, MatchResult * remote_result, CContext * oContext) const
        {
            ...
            static regexpT <CHART> rtoreg(rtoptn);
        ...
        template <class CHART> inline void regexpT <CHART> :: ReleaseString(CHART * tstring)
        {
            if(tstring != 0) free(tstring);
        }

        template <class CHART> inline void regexpT <CHART> :: ReleaseContext(CContext * pContext)
        {
        ...
        typedef regexpT <unsigned short> regexpW;

        #if defined(_UNICODE) || defined(UNICODE)
            typedef regexpW regexp;
        #else
            typedef regexpA regexp;
        #endif
